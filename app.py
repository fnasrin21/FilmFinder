...
def get_movies(genre, rating, language):
    ...
    try:
        ...
        response.raise_for_status()  # This raises an error for any link issues

        data = response.json()
        movies = []
        for result in data.get('results', [])[:10]:  #Shows top 10 film recs
            movies.append({
                'title': result['title'],
...
def recommend():
    ...
    # If a mood is selected, map it to a genre, this overrides the mood choice
    if mood:
        genre = MOOD_TO_GENRE.get(mood.lower(), genre)

    # Map genre name to TMDb genre ID from API
